...
import { setCurrentReport, clearCurrentReport } from '../store/slices/reportSlice';
...
const ReportDetail: React.FC = () => {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      // 1. 먼저 세션 스토리지에서 리포트 데이터 확인
      const sessionReport = sessionStorage.getItem('current_report');
      if (sessionReport) {
        try {
          const parsedReport = JSON.parse(sessionReport);
          if (parsedReport && parsedReport.report_id === reportId) {
            console.log('세션 스토리지에서 리포트 데이터 로드:', parsedReport.report_id);
            dispatch(setCurrentReport(parsedReport));
            setIsLoading(false);
            setHasAttemptedLoad(true);
            return;
          }
        } catch (e) {
          console.warn('세션 스토리지의 리포트 데이터 파싱 오류:', e);
        }
      }
      
      // 2. 세션 스토리지에 없으면 API에서 로드
  ...
  useEffect(() => {
    ...
    // 컴포넌트 언마운트 시 세션 스토리지에서 리포트 데이터 제거
    return () => {
                      ...
                      {host.ports.map((port: PortInfo, portIndex: number) => (
                        <div key={portIndex} className="p-4 border rounded-lg bg-gray-50/50">
                          <h5 className="text-base font-semibold mb-3 flex items-center gap-2">
                            ...
                            <span>포트 {port.port}/{port.protocol || 'tcp'}: {port.service || '알 수 없음'}</span>
                          ...
                          {port.product && (
                            <div className="text-sm text-muted-foreground mb-4 pl-6">
                              ...
                              {port.extrainfo && <p><strong>Extra Info:</strong> {port.extrainfo}</p>}
                            </div>
                          )}
                                    ...
                                    <AccordionItem key={vulnIndex} value={`vuln-${portIndex}-${vulnIndex}`} className="border-b last:border-b-0 overflow-hidden">
                                      <AccordionTrigger className="px-3 py-2 hover:no-underline hover:bg-muted/30">
                                        <div className="flex items-center justify-between w-full">
                                          ...
                                          >
                                            {typeof vuln.cvss_score === 'number' ? vuln.cvss_score.toFixed(1) : vuln.cvss_score}
                                          </Badge>
                                        </div>
                                      </AccordionTrigger>
                                      <AccordionContent className="px-3 py-2 bg-muted/10">
                                        <div className="text-xs text-muted-foreground max-h-[200px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">{vuln.description || vuln.title || '설명 없음'}</div>
                                      </AccordionContent>
                                    </AccordionItem>
                                  ))}
                                </Accordion>
